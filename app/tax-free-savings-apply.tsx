...
import {
  isValidSAID, extractDobFromId, normalizePhone,
  isValidPhone, sendWhatsAppOtp, verifyWhatsAppOtp,
} from '../lib/productUtils';
import type { OtpPurpose } from '../lib/productUtils';
import ProfileGuard from '../components/ProfileGuard';

const STEPS = ['Personal', 'Savings Plan', 'Consent & OTP', 'Review'];
const OTP_PURPOSE: OtpPurpose = 'tax_free_savings_authorization';
...
export default function TaxFreeSavingsApplyScreen() {
  ...
  const validate = (): boolean => {
    ...
    if (step === 1) {
      ...
      const annual = Number(monthlyContribution) * 12;
      if (annual > 36000) { setError('Annual max is R36,000 (R3,000/month).'); return false; }
      return true;
    }
    if (step === 2) {
      if (!popiaConsent || !tfsConsent) { setError('Accept both consents.'); return false; }
  ...
  const handleSendOtp = async () => {
    if (!popiaConsent || !tfsConsent) { setError('Accept both consents first.'); return; }
    setLoading(true);
    try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await setDoc(consentRef, {
        consentId: consentRef.id, consentType: 'tax_free_savings_authorization',
      ...
      await setDoc(productRef, {
        productApplicationId: productRef.id, productType: 'tax_free_savings',
        productName: 'Tax Free Savings', productDescription: `TFSA — R${Number(monthlyContribution).toLocaleString()}/month`,
        ...
        reference: `TFS-${uid.slice(0, 6)}-${consentRef.id}`, consentId: consentRef.id,
      ...
      console.error('[TaxFreeSavings] Submit failed:', err);
      ...
      <SafeAreaView style={st.container} edges={['bottom']}>
        <ScrollView contentContainerStyle={st.scrollContent}>
          <View style={st.stepContainer}>
            ...
            <Text style={st.desc}>Your Tax Free Savings application has been received.</Text>
    ...
    <SafeAreaView style={st.container} edges={['bottom']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={st.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {step === 1 && (
            <View style={st.stepContainer}>
              <View style={st.iconCircle}><Ionicons name="cash" size={28} color={Colors.primary.blue} /></View>
              <Text style={st.title}>Savings Plan</Text>
              <TextInput style={st.input} placeholder="Monthly Contribution (R) — max R3,000" placeholderTextColor={Colors.text.light} value={monthlyContribution} onChangeText={setMonthlyContribution} keyboardType="number-pad" />
              <TextInput style={st.input} placeholder="Savings Term (years)" placeholderTextColor={Colors.text.light} value={savingsTerm} onChangeText={setSavingsTerm} keyboardType="number-pad" />
              <View style={[st.card, { backgroundColor: Colors.primary.blue + '08' }]}>
                <Text style={{ fontSize: 13, color: Colors.text.secondary, lineHeight: 19 }}>
                  Tax Free Savings contributions are limited to R36,000 per year (R3,000/month) with a lifetime limit of R500,000. All growth is tax-free.
                </Text>
              </View>
            </View>
          )}
          ...
          {step === 2 && (
            <View style={st.stepContainer}>
              ...
              <View style={st.card}>
                ...
                <TouchableOpacity style={st.checkboxRow} onPress={() => setTfsConsent(!tfsConsent)}>
                  <View style={[st.checkbox, tfsConsent && st.checkboxChecked]}>{tfsConsent && <Ionicons name="checkmark" size={14} color="#fff" />}</View>
                  <Text style={st.checkboxLabel}>I authorise MyBenefitz to process my Tax Free Savings application.</Text>
                </TouchableOpacity>
              </View>
              {otpVerified ? (
                ...
                <TouchableOpacity style={[st.actionBtn, (!popiaConsent || !tfsConsent) && st.actionBtnDisabled]} onPress={handleSendOtp} disabled={!popiaConsent || !tfsConsent || loading}>
          ...
          {step === 3 && (
            <View style={st.stepContainer}>
              ...
              <View style={st.card}>
                ...
                {savingsTerm ? <><Text style={st.reviewLabel}>Savings Term</Text><Text style={st.reviewValue}>{savingsTerm} years</Text></> : null}
              </View>
            </View>
          )}
