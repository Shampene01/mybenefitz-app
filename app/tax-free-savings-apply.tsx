...
const TERM_OPTS = ['5', '10', '15', '20', '25', '30'];
...
export default function TaxFreeSavingsApplyScreen() {
  ...
  const [tfsConsent, setTfsConsent] = useState(false);
  ...
  const progressTo = useCallback((step: StepKey, extra?: Record<string, string>) => {
    ...
    switch (step) {
      ...
      case 'savings_term': addBot("How long would you like to save? (years) 📅\n\n💡 Annual limit: R36,000 • Lifetime limit: R500,000\nAll growth is <b>100% tax-free</b>!"); break;
      ...
      case 'amount': {
        ...
        const max = getTfsMaxContribution(freq);
        const fl = CONTRIBUTION_FREQUENCY_OPTIONS.find(f => f.id === freq)?.label || freq;
        addBot(`${fl} contribution amount? (R)\n\n⚠️ Max R${max.toLocaleString()} per ${fl.toLowerCase()} (R36,000/year limit).`);
        break;
      }
  ...
  const handleAvatarSelect = useCallback((av: AvatarProfile) => {
    ...
    addBot(`Hey ${firstName}! I'm <b>${av.name}</b> 💰 Let's set up your Tax Free Savings — grow your wealth tax-free!`);
  ...
  const handleButtonClick = useCallback((optId: string, label: string, field?: string) => {
    ...
    if (field === 'maritalStatus') { progressTo('savings_term', { maritalStatus: optId }); return; }
    if (field === 'savingsTerm') { progressTo('contribution_type', { savingsTerm: optId }); return; }
  ...
  const handleTextSubmit = useCallback(() => {
    ...
    if (currentStep === 'amount') {
      ...
      const max = getTfsMaxContribution(freq);
      if (Number(n) > max) { addBot(`⚠️ Max R${max.toLocaleString()} for ${CONTRIBUTION_FREQUENCY_OPTIONS.find(f => f.id === freq)?.label || freq}. Try again.`); return; }
      const fl = CONTRIBUTION_FREQUENCY_OPTIONS.find(f => f.id === freq)?.label || freq;
      addBot(`✅ R ${Number(n).toLocaleString()} per ${fl}`);
    ...
    if (currentStep === 'start_date') { progressTo('confirm_address', { startDate: v }); return; }
  ...
  const handleSendOtp = useCallback(async () => {
    ...
    try {
      const r = await sendWhatsAppOtp(normalizePhone(ph), 'tax_free_savings_authorization' as OtpPurpose);
  ...
  const handleSubmit = useCallback(async (otp?: string) => {
    ...
    try {
      ...
      const appData = {
        ...
        savingsTerm: data.savingsTerm ? Number(data.savingsTerm) : null,
        ...
        annualLimit: 36000, lifetimeLimit: 500000,
      ...
      await setDoc(cR, { consentId: cR.id, consentType: 'tax_free_savings_authorization', fullName: full, surname: pS, idNumber: idN, clientPhone: phone, popiaConsent: true, whatsAppContactConsent: true, otpVerified: true, otpCode: otpId, otpVerifiedAt: now, consentGrantedAt: now, messageId: otpMsgId || null, purpose: 'Tax Free Savings Authorization', status: 'active', channel: 'chat_mode', createdAt: now });

      await submitProductApplication({ uid, productType: 'tax_free_savings', productName: 'Tax Free Savings', productDescription: `TFSA — R${Number(data.amount || 0).toLocaleString()} ${freq}`, status: 'applied', statusLabel: 'Application Submitted', applicationData: appData as unknown as Record<string, unknown>, consent: { consentId: cR.id, consentType: 'tax_free_savings_authorization', otpVerified: true, otpCode: otpId, otpMessageId: otpMsgId || null, otpVerifiedAt: now, consentGrantedAt: now, messageId: otpMsgId || null }, reference: `TFS-${uid.slice(0, 6)}-${cR.id}`, idNumber: idN, waId: userProfile.waId || null, email: userProfile.email, clientName: full, amount: data.amount ? Number(data.amount) : 0, registrationType, referrerCode, referredBy });

      setCurrentStep('done');
      addBot(`🎉 Submitted!\n\nRef: TFS-${uid.slice(0, 6)}-${cR.id}\n💰 R ${Number(data.amount || 0).toLocaleString()} ${freq}\n📅 ${data.savingsTerm || '?'} year term\n\nA consultant will be in touch. Track in "My Products".`);
    } catch (e) { console.error('[TFS]', e); setError('Failed.'); setCurrentStep('consent_step'); }
  ...
  const buttonConfigs: Partial<Record<StepKey, { options: { id: string; label: string; icon?: string }[]; field: string }>> = {
    ...
    savings_term: { options: TERM_OPTS.map(t => ({ id: t, label: `${t} years` })), field: 'savingsTerm' },
          ...
          {!avatarChosen && (
            <ScrollView contentContainerStyle={styles.avatarScroll}>
              <View style={styles.avatarCard}>
                <View style={styles.avatarBadge}><Ionicons name="cash" size={13} color={AC} /><Text style={styles.avatarBadgeText}>Tax Free Savings — Chat Mode</Text></View>
                <Text style={styles.avatarTitle}>Choose Your Assistant</Text>
                <Text style={styles.avatarSubtitle}>Pick who you'd like to guide you through your TFSA application.</Text>
            ...
            <View style={styles.chatWrap}>
              <View style={styles.chatHeader}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.headerSub}>Tax Free Savings — Application</Text>
                </View>
              ...
              <ScrollView ref={scrollRef} style={styles.msgScroll} contentContainerStyle={styles.msgScrollContent} showsVerticalScrollIndicator={false}>
                ...
                {currentStep === 'consent_step' && (
                  <View style={styles.inlineCard}>
                    ...
                    <TouchableOpacity style={styles.checkRow} onPress={() => setTfsConsent(!tfsConsent)}><Ionicons name={tfsConsent ? 'checkbox' : 'square-outline'} size={20} color={AC} /><Text style={styles.checkText}>I authorise TFSA application processing.</Text></TouchableOpacity>
                    {mPh ? <Text style={{ fontSize: 12, color: '#6b7280', marginTop: 8 }}>Code sent to {mPh}</Text> : null}
                    {error ? <Text style={styles.errorText}>{error}</Text> : null}
                    <TouchableOpacity style={[styles.confirmBtn, !(popiaConsent && tfsConsent) && { opacity: 0.5 }]} onPress={handleSendOtp} disabled={!(popiaConsent && tfsConsent) || isLoading}>
...
const styles = StyleSheet.create({
  ...
  bubbleUser: { backgroundColor: '#e0e7ff', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, borderBottomRightRadius: 2, maxWidth: '75%' },
  ...
  optionBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, borderWidth: 1.5, borderColor: '#c7d2fe', backgroundColor: ACBG },
